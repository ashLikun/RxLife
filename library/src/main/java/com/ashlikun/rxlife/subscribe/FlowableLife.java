...
public class FlowableLife<T> extends RxSource<FlowableSubscriber<? super T>> {
    ...
    public FlowableLife(Flowable<T> upStream, Scope scope, boolean onMain) {
        super(scope, onMain);
    ...
    @Override
    public final Disposable subscribe(FlowableSubscriber<? super T> observer) {
        ObjectHelper.requireNonNull(observer, "s is null");
        try {
            Subscriber<? super T> z = RxJavaPlugins.onSubscribe(upStream, observer);
    ...
    private void subscribeActual(Subscriber<? super T> s) {
        ...
        upStream.onTerminateDetach().subscribe(new LifeSubscriber<>(s, getScope()));
    }
}
