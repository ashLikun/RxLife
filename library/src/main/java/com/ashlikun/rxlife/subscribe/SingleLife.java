...
import com.ashlikun.rxlife.lifecycle.LifeSingleObserver;
...
public class SingleLife<T> extends RxSource<SingleObserver<? super T>> {
    ...
    public SingleLife(Single<T> upStream, Scope scope, boolean onMain) {
        super(scope, onMain);
    ...
    @Override
    public final Disposable subscribe(SingleObserver<? super T> observer) {
    ...
    private void subscribeActual(SingleObserver<? super T> observer) {
        ...
        upStream.onTerminateDetach().subscribe(new LifeSingleObserver<>(observer, getScope()));
    }
}
